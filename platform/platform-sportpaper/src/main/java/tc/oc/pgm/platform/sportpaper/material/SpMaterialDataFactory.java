...
public class SpMaterialDataFactory implements MaterialData.Factory {
  int ENCODED_NULL_MATERIAL = -1;

  @Override
  public MaterialData from(Material material) {
    return new SpMaterialData(new org.bukkit.material.MaterialData(material));
  }
  ...
  public MaterialData decode(int encoded) {
    ...
    if (material.getData() == org.bukkit.material.MaterialData.class) {
      ...
      return new SpMaterialData(material.getNewData(decodeMetadata(encoded)));
    }
  }
  ...
  public MaterialData getTo(EntityChangeBlockEvent event) {
    return new SpMaterialData(new org.bukkit.material.MaterialData(event.getTo(), event.getData()));
  }
