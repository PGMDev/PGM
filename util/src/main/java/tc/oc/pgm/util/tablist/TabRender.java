...
public class TabRender {
  ...
  //        .add(NMSHacks.playerListPacketData(this.updatePacket, entry.getId(),
  // renderedDisplayName));
  //
  ...
  public void finish() {
    //    if (!NMSHacks.getPlayerInfoDataList(this.removePacket).isEmpty())
    // this.send(this.removePacket);
    //    if (!NMSHacks.getPlayerInfoDataList(this.addPacket).isEmpty()) this.send(this.addPacket);
    //    if (!NMSHacks.getPlayerInfoDataList(this.updatePacket).isEmpty())
  ...
  public void updatePing(TabEntry entry, int index) {
    ...
    //        .add(NMSHacks.playerListPacketData(this.updatePingPacket, entry.getId(),
    // entry.getPing()));
  }
  ...
  public void updateFakeEntity(TabEntry entry, boolean create) {
    ...
    //                entityId,
    //                entry.getId(),
    //                new Location(this.view.getViewer().getWorld(), 0, Integer.MAX_VALUE / 2, 0, 0,
    // 0),
    //                player));
    //      } else {
